...
class NaiveAll2AllManager(All2AllManagerBase):
    ...
    def dispatch(
        ...
        if extra_tensors is not None:
            print("calling extra tensors!!!!!!!!!!!!!!!!!!!!")
            gathered_extras = []
            for t in extra_tensors:
                gathered_extras.append(
                    self.naive_multicast(
                        t, cu_tokens_across_sp_cpu, is_sequence_parallel
                    )
                )
            return hidden_states, router_logits, gathered_extras
